...
class LLM_Summarize:
    ...
    def __init__(self, llm_token):
        self.llm = ChatOpenAI(model_name="gpt-4o-mini",temperature=0.1, openai_api_key=llm_token)
    ...
    def summarize_repo(self, code_list):
        ...
        logger.info("Running LLM (manual rate control)")

        results = []
        for i, doc in enumerate(split_docs):
            try:
                # run the "map" stage manually for each chunk
                partial = map_reduce_chain.llm_chain.run(codes=doc.page_content)
                results.append(partial)
                logger.info(f"Processed chunk {i+1}/{len(split_docs)}")
                time.sleep(20)  # wait 20 seconds before next API call (3 RPM limit)
            except Exception as e:
                logger.warning(f"Chunk {i+1} failed: {e}")
                time.sleep(25)  # back off a bit and continue
                continue

        # now reduce (combine) all partial summaries into one final summary
        result = map_reduce_chain.reduce_documents_chain.combine_documents_chain.llm_chain.run(
            summary_list="\n".join(results)
        )
